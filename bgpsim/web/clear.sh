...
#docker ps -a | tail -n +2 | awk '{print $NF}' | while read l; do echo $l; docker exec $l bash -c "eval \"ip route add 192.168.7.0/24 \$(ip route | grep \"^2.43.0.0/\" | cut -d' ' -f2,3,4,5)\""; done
